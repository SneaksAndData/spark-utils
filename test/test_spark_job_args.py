...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "file://data/yahoo-exchange-rates-2.json",
            "yahoo-exchange-rates-2",
            id="1",
        ),
        pytest.param(
            ["yahoo-exchange-rates-1|file://data/yahoo-exchange-rates-1.json|json"],
            "file://data/yahoo-exchange-rates-1.json",
            "yahoo-exchange-rates-1",
            id="2",
        ),
    ],
)
def test_parse_source(source, expected, key):
    sa = SparkJobArgs()
    sa.parse(["--source", *source, "--output", "exchange-rates|file://data/test-result.json|json", "--overwrite"])
...
@pytest.mark.parametrize(
    "columns",
    [
        pytest.param(["colA", "colB", "colC"], id="1"),
        pytest.param(["colA"], id="2"),
    ],
)
def test_parsed_args(columns):
    sa = SparkJobArgs()
    sa.new_arg("--columns", type=str, nargs="+", default=[], help="Unit Test")
    sa.parse(["--columns", *columns])
...
def test_new_arg_chain():
    sa = SparkJobArgs()
    parsed = (
        sa.new_arg("--arg1", type=str, help="Argument 1")
...
@pytest.mark.parametrize(
    "overwrite,expected",
    [
        pytest.param(False, False, id="1"),
        pytest.param(True, True, id="2"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("testabcd", "testabcd"),
        pytest.param("", None),
    ],
)
def test_encrypted_arg(value, expected_value):
    ...
    arglist = ["--encrypted-arg", f"'{encrypted_value}'"]
